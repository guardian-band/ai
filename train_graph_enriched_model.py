...
from src.models.unified_polypharmacy_gnn import UnifiedPolypharmacyGNN
...
class GraphEnrichedDataset(Dataset):
...
def main():
    ...
    print("="*65)
    print("TRAINING GRAPH-ENRICHED POLYPHARMACY AI MODEL")
    print("Pre-Computed 2-Hop Drug→Protein→Drug Message Passing")
    print("="*65)
    
    # Use graph-enriched features
    drug_feat_df = pd.read_parquet(os.path.join(art_dir, "drug_features_graph_enriched.parquet"))
    ...
    feat_cols = [c for c in drug_feat_df.columns if c.startswith('f_') or c.startswith('g_') or c.startswith('gs_')]
    in_features = len(feat_cols)
    print(f"Graph-Enriched Input Feature Dimension: {in_features}")
    ...
    def build_matrices(pairs_df):
        pairs_list, y_organ_mat, y_spec_mat = [], [], []
    ...
    train_dataset = GraphEnrichedDataset(train_pairs, y_train_org, y_train_spec, drug_feature_dict)
    test_dataset = GraphEnrichedDataset(test_pairs, y_test_org, y_test_spec, drug_feature_dict)
    ...
    print("\nStarting Graph-Enriched GPU Training Loop...")
    for epoch in range(1, epochs + 1):
        ...
        for feat_a, feat_b, target_org, target_sp in train_loader:
            ...
            loss = criterion(logits_org, target_org) + criterion(logits_sp, target_sp)
            loss.backward()
            optimizer.step()
    ...
    # Evaluation
    print("\nEvaluating Graph-Enriched Model on Test Pairs...")
    model.eval()
    ...
    print("\n" + "*"*65)
    print("🏆 GRAPH-ENRICHED POLYPHARMACY AI MODEL - EVALUATION REPORT")
    print("="*65)
    ...
    model_save_path = os.path.join(art_dir, "champion_graph_enriched_gnn.pt")
    torch.save(model.state_dict(), model_save_path)
    print(f"\nSaved Graph-Enriched Model to {model_save_path}")
