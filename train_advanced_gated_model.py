...
from src.models.advanced_gated_polypharmacy_gnn import AdvancedGatedPolypharmacyGNN, AsymmetricLoss
...
class GatedPolypharmacyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # SIDER pair prior: union of individual mono-drug side effects
...
def main():
    ...
    print("="*65)
    print("TRAINING ADVANCED HIERARCHICAL GATED MODEL")
    print("Hierarchical Top-Down Gating + SIDER 4.1 Priors + Asymmetric Loss")
    print("="*65)
    ...
    train_dataset = GatedPolypharmacyDataset(train_pairs, y_train_org, y_train_spec, drug_feature_dict, sider_prior_dict)
    test_dataset = GatedPolypharmacyDataset(test_pairs, y_test_org, y_test_spec, drug_feature_dict, sider_prior_dict)
    ...
    model = AdvancedGatedPolypharmacyGNN(
    ...
    asl_criterion = AsymmetricLoss(gamma_neg=2.0, gamma_pos=0.0, clip=0.05)
    bce_criterion = nn.BCEWithLogitsLoss()
    
    print("\nStarting Optimized Gated Training Loop...")
    for epoch in range(1, epochs + 1):
        ...
        for feat_a, feat_b, pair_prior, target_org, target_sp in train_loader:
            ...
            loss_org = bce_criterion(logits_org, target_org)
            loss_sp = asl_criterion(logits_sp, target_sp)
    ...
    print("\nEvaluating Gated Model on Test Pairs...")
    model.eval()
    ...
    print("\n" + "*"*65)
    print("🏆 GATED MULTI-TASK POLYPHARMACY AI MODEL - EVALUATION REPORT")
    print("="*65)
    ...
    model_save_path = os.path.join(art_dir, "champion_gated_polypharmacy_ai.pt")
    torch.save(model.state_dict(), model_save_path)
    print(f"\nSaved Champion Gated Model weights to {model_save_path}")
