...
class UnifiedPolypharmacyDataset(Dataset):
...
def main():
    ...
    print("="*60)
    print("TRAINING UNIFIED MASTER POLYPHARMACY AI MODEL")
    print("Integrating DrugBank + PrimeKG + ChEMBL + BioSNAP")
    print("="*60)
    ...
    train_dataset = UnifiedPolypharmacyDataset(train_pairs, y_train_org, y_train_spec, drug_feature_dict)
    test_dataset = UnifiedPolypharmacyDataset(test_pairs, y_test_org, y_test_spec, drug_feature_dict)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 5 == 0 or epoch == epochs:
            ...
            print(f"Epoch {epoch:02d}/{epochs:02d} - Multi-Task Loss: {avg_loss:.4f} (LR: {scheduler.get_last_lr()[0]:.6f})")
            
    # Comprehensive Evaluation
    print("\nEvaluating Unified Model on Test Pairs...")
    model.eval()
    ...
    print("\n" + "*"*65)
    print("🏆 UNIFIED MASTER POLYPHARMACY AI MODEL - EVALUATION REPORT")
    print("="*65)
    ...
    print(f"  Spesifik Yan Etki Macro AUPRC:           {np.mean(sp_auprc):.4f}")
    print(f"  Spesifik Yan Etki Precision@5:           {p_at_5:.4f}")
    print("*"*65)
    
    model_save_path = os.path.join(art_dir, "unified_polypharmacy_ai.pt")
    torch.save(model.state_dict(), model_save_path)
    print(f"\nSaved Unified Master Model weights to {model_save_path}")
